...
const toRelativeParts = (date: string) => {
  const elapsedMs = Math.max(Date.now() - new Date(date).getTime(), 0)
...
type RelativeTimeProps = { value: string | null | undefined; className?: string }

/** Null when the source carries no date — the caller renders nothing then. */
export const RelativeTime = ({ value, className }: RelativeTimeProps) => {
  if (!value) return null

  const { value: relativeValue, unit } = toRelativeParts(value)

  return (
    <span className={className}>
      <FormattedRelativeTime value={relativeValue} unit={unit} numeric="auto" />
    </span>
  )
}
