...
export const IdentitySection = ({
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap gap-5">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-center text-[11px] text-muted-foreground">
            <FormattedMessage id="coach.profile.identity.avatar.hint" />
          </p>
        </div>
        <div className="grid min-w-60 flex-1 grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="flex flex-col gap-1.5">
            <Label>
              <FormattedMessage id="coach.profile.identity.firstName.label" /> <span className="text-primary">*</span>
            </Label>
            <Input value={firstName} onChange={(e) => onFirstNameChange(e.target.value)} />
          </div>
          <div className="flex flex-col gap-1.5">
            <Label>
              <FormattedMessage id="coach.profile.identity.lastName.label" /> <span className="text-primary">*</span>
            </Label>
            <Input value={lastName} onChange={(e) => onLastNameChange(e.target.value)} />
          </div>
          <div className="flex flex-col gap-1.5 sm:col-span-2">
            <Label>
              <FormattedMessage id="coach.profile.identity.phone.label" />
            </Label>
            <Input
              type="tel"
              placeholder={intl.formatMessage({ id: 'coach.profile.identity.phone.placeholder' })}
              value={phone}
      ...
      <div className="flex flex-col gap-1.5">
        <Label>
          <FormattedMessage id="coach.profile.identity.bio.label" />
        </Label>
        <p className="text-[11.5px] text-muted-foreground">
          <FormattedMessage id="coach.profile.identity.bio.hint" />
        </p>
        <Textarea
          className="h-[140px] resize-y"
          maxLength={MAX_BIO_LENGTH}
          value={bio}
          onChange={(e) => onBioChange(e.target.value)}
        />
        <p className="self-end text-xs text-muted-foreground">
          <FormattedMessage id="coach.profile.identity.bio.counter" values={{ count: bio.length, max: MAX_BIO_LENGTH }} />
        </p>
      </div>
    </div>
  )
