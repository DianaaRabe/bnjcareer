...
export const useCvOptimizationResult = () => {
  ...
  const downloadPdf = useCallback(async (element: HTMLElement | null) => {
    if (!element) return
    ...
    try {
      ...
      // Fallback: open the browser print dialog with the rendered DOM.
      const printWindow = window.open('', '_blank')
      if (printWindow) {
        printWindow.document.write(
          `<!DOCTYPE html><html><head><title>CV</title></head><body>${element.outerHTML}</body></html>`,
        )
